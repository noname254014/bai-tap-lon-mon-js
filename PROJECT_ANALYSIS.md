...
{
    id: number,
    position: number,           // Continuous floor position
    velocity: number,           // Current velocity (fl/s)
    acceleration: number,       // Current acceleration (fl/s²)
    direction: 'up'|'down'|'none',
    doorState: DOOR_STATE,
    passengers: Passenger[],
    targetFloors: number[],     // Sorted target list
    currentLoad: number,        // Total passenger weight (kg)
    status: ELEVATOR_STATUS,
    faultStatus: FAULT_TYPE,
    stuckTimer: number,         // Time stuck at 0 velocity
    doorTimer: number,          // Door sequence timer
    phase: ELEVATOR_PHASE,
    _doorPhase: string|null,    // Door sub-state
    _lastPosition: number,      // Previous position
    _floorStopHandled: number,  // Last floor stop processed
    idleTimer: number           // Time spent idle (for pre-positioning)
}
```
...
```javascript
{
    ...
    weight: number,             // kg (Gaussian distribution)
